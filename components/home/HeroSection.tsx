...
export function HeroSection({ article }: { article: Article }) {
  ...
  const dek = useTranslatedText(article.dek);

  return (
    <section aria-labelledby="hero-headline" className="border-b border-hairline">
      <Link href={`/article/${article.slug}`} className="group grid gap-0 md:grid-cols-[1.4fr_1fr]">
        ...
        <div className="flex flex-col justify-center gap-4 bg-ink-950 px-6 py-8 md:px-10 md:py-0">
          ...
          <p className="text-base leading-relaxed text-ink-100 md:text-lg">{dek}</p>
          <ArticleMeta
